...
- queryKey: ["posts"]:
  queryKey는 이 쿼리의 고유한 식별자 역할을 한다. 서버에서 데이터를 패칭할 때, React Query는 이 키를 기준으로 캐싱을 처리하고, 데이터를 다시 가져와야 하는지 판단한다. 여기서 ["posts"]는 "posts"라는 이름으로 데이터를 구분한다.

- queryFn: fetchPost:
  queryFn은 데이터를 가져오는 함수이다. fetchPost 함수는 서버에서 데이터를 가져오는 비동기 함수이며, 이 예시에서는 fetchPost를 통해 "posts" 데이터를 가져옵니다. fetchPost는 Promise를 반환하는 함수여야 한다.

- retry: 1:
  이 설정은 요청이 실패했을 때 재시도 횟수를 지정한다. 여기서 1은 요청이 실패했을 때 1회 재시도한다는 의미이다. 기본값은 3회이다.

- select: (data) => data.data:
  select는 가져온 데이터를 변환하거나 가공할 수 있는 함수이다. select 함수는 쿼리가 성공적으로 완료된 후 실행된다. 여기서는 data 객체에서 data.data만을 선택해서 반환한다. 서버 응답이 data: [...] 형태로 오는 경우에 특정 속성만 선택할 때 사용한다.

- staleTime : 데이터를 캐시한 후 얼마 동안 데이터가 신선하다고 간주될지를 설정하는 옵션이다. 이를 통해 데이터 요청을 최적화하고 성능을 향상시키며, 사용자 경험을 개선할 수 있다. 1000 : 1초
  staleTime: 10000 로 하면 , 데이터가 캐시된 후 10초 동안은 React Query가 자동으로 새 데이터를 요청하지 않고, 캐시된 데이터를 사용한다.
  즉, 정리를 하면 <br/>

**staleTime의 역할**

- **데이터 신선도 유지**:
  staleTime은 데이터를 캐시한 후 얼마 동안 그 데이터를 **"fresh"**하다고 간주할지를 정의한다. 이 시간 동안은 React Query가 데이터를 자동으로 다시 요청하지 않다.
- **재요청 최적화**:
  staleTime이 지나면 데이터는 **"stale"**으로 간주되며, React Query는 다음에 해당 데이터가 필요할 때 서버로부터 새로운 데이터를 요청한다.
...
useQueries는 React Query의 훅으로, 여러 쿼리를 동시에 처리하고 관리하는 데 사용된다. 각 쿼리는 useQuery와 비슷하지만, 여러 쿼리의 결과를 배열 형태로 반환한다.
