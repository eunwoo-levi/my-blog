...
현재 AWS CodeBuild + CodePipeline + Lambda로 배포된 전체 아키텍처는 아래 그림과 같다.

![](https://velog.velcdn.com/images/eunwoo1341/post/0bed4691-1765-4480-8f43-e92fe2c0ee95/image.png)

GitHub Actions에서 PR 생성/브랜치 병합 시 코드 품질 검증과 통합 테스트를 자동 수행하는 것부터 시작한다.
이후 CodePipeline이 dev/prod 브랜치를 모니터링하다가 새 커밋을 감지하면 배포를 자동으로 트리거하고, CodeBuild에서 소스 코드를 빌드하면서 환경 변수를 주입해 정적 파일을 생성한다.
생성된 산출물은 CodeDeploy를 통해 S3 버킷에 업로드되며, 업로드 완료 이벤트로 Lambda가 실행되어 CloudFront 캐시를 자동 무효화(invalidation) 함으로써 최신 버전이 즉시 반영된다.
최종적으로 사용자는 도메인을 통해 CloudFront에서 정적 파일을 제공받고, 클라이언트에서 백엔드 서버와 API 통신을 수행한다.
...
위 문제로부터 `"Access Key를 제공하지 않는 환경에서도, 저렴한 비용으로 사람이 서버에 접속하지 않고 자동으로 배포되는 CI/CD 파이프라인을 만들자!"`s라는 목표를 세우게 되었고,
...
이제, CodePipeline 구축부터 Lambda 구축까지 단계별로 자세히 살펴보자.
